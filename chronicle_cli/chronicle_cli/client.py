...
async def api_request(method: str, path: str, **kwargs) -> httpx.Response:
    ...
    if not context:
        console.print("[#ef4444]X[/#ef4444] Not configured. Run 'chronicle init' first.")
    ...
    try:
        ...
        console.print(f"[#ef4444]X[/#ef4444] Could not connect to {context['backend_url']}. Is the server running?")
        raise typer.Exit(code=1)
    except httpx.HTTPStatusError as e:
        status = e.response.status_code
        if status == 401:
            console.print("[#ef4444]X[/#ef4444] Authentication failed. Check your API key in ~/.chronicle/config.json")
            raise typer.Exit(code=1)
        elif status >= 500:
            console.print("[#ef4444]X[/#ef4444] Chronicle backend error. Check server logs.")
            raise typer.Exit(code=1)
